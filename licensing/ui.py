...
class LicenseDialog(tk.Toplevel):
    def __init__(self, parent, on_license_change=None):
        ...
        ttk.Button(root, text="Copy", command=self._copy_device_id).grid(
            row=2, column=2, sticky="w", padx=(8, 0), pady=(8, 4)
        )

        ttk.Label(root, text="License Key").grid(row=3, column=0, sticky="w", pady=(10, 4))
        self.license_key_var = tk.StringVar(value="")
        self.license_key_entry = ttk.Entry(root, textvariable=self.license_key_var)
        self.license_key_entry.grid(row=3, column=1, sticky="ew", pady=(10, 4))
        ttk.Button(root, text="Activate Key", command=self._activate_online_key).grid(
            row=3, column=2, sticky="w", padx=(8, 0), pady=(10, 4)
        )

        ttk.Label(root, text="License (string or file)").grid(row=4, column=0, sticky="w", pady=(10, 4))
        ...
        self.license_text.grid(row=5, column=0, columnspan=3, sticky="nsew", pady=(0, 10))

        ttk.Label(root, text="Emergency code").grid(row=6, column=0, sticky="w", pady=(0, 4))
        self.emergency_code_var = tk.StringVar(value="")
        self.emergency_entry = ttk.Entry(root, textvariable=self.emergency_code_var, show="*")
        self.emergency_entry.grid(row=6, column=1, sticky="ew", pady=(0, 4))
        ttk.Button(root, text="Activate emergency", command=self._activate_emergency_code).grid(
            row=6, column=2, sticky="w", padx=(8, 0), pady=(0, 4)
        )

        btn_row = ttk.Frame(root)
        btn_row.grid(row=8, column=0, columnspan=3, sticky="e")
        ttk.Button(btn_row, text="Activate Token", command=self._activate_license_text).pack(side="left", padx=(0, 8))
    ...
    def _activate_online_key(self) -> None:
    ...
    def _activate_license_text(self) -> None:
    ...
    def _activate_emergency_code(self) -> None:
        code = (self.emergency_code_var.get() or "").strip()
        if not code:
